...
void Single_Altitude_PID_Calculation(PIDSingle* axis, float set_point_altitude, float measured_altitude)
{

	axis->reference = set_point_altitude;
	axis->meas_value = measured_altitude;

	axis->error = axis->reference - axis->meas_value;	//Define error

	axis->p_result = axis->error * axis->kp;			//Calculate P result
	...
	axis->d_result = axis->error_deriv * axis->kd;	//Calculate D result
	axis->pid_result = axis->p_result + axis->i_result + axis->d_result; //Calculate PID result

	axis->meas_value_prev =measured_altitude;
	/***************************************************************/
}
